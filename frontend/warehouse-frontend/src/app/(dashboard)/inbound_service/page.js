...
import { getInboundShipments, getAllReceipts, getAllReceiptItems, updateShipmentStatus } from "@/services/inbound_service/inboundApi";
        ...
        <TableContainer>
          <Table>
            <TableHead sx={{ bgcolor: "#f8fafc" }}>
              <TableRow>
                {tabValue === 0 && <><TableCell>Supplier</TableCell><TableCell>Product</TableCell><TableCell>Status</TableCell><TableCell align="right">Action</TableCell></>}
                {tabValue === 1 && <><TableCell>GRN Number</TableCell><TableCell>Date</TableCell><TableCell>Status</TableCell></>}
                {tabValue === 2 && <><TableCell>Item ID</TableCell><TableCell>Product Name</TableCell><TableCell>Qty Received</TableCell><TableCell>Quality</TableCell></>}
              </TableRow>
            </TableHead>
            <TableBody>
              {data.map((row) => (
                <TableRow key={row.id} hover>
                  {tabValue === 0 && (
                    <>
                      ...
                      <TableCell align="right">
                        <Button size="small" onClick={() => handleStatusChange(row.id, row.status)}>Update</Button>
                      </TableCell>
                    </>
                  )}
                  {tabValue === 1 && (
                    <>
                      <TableCell sx={{ color: "#6366f1", fontWeight: 600 }}>{row.grnNumber || row.receiptNumber}</TableCell>
                      <TableCell>{new Date(row.receiptDate || row.receivedAt).toLocaleDateString()}</TableCell>
                  ...
                  {tabValue === 2 && (
                    <>
                      ...
                      <TableCell>
                        <Chip label={row.qualityStatus || "PENDING"} size="small" variant="outlined" />
                      </TableCell>
                    </>
                  )}
                </TableRow>
